# ARCHITECTURE: Conflicts, Gifts and PAD Register (`conflicts-gifts-pad-register`)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`CONFLICTSPAD_PROFILE`) swaps the
...
## Layout (`src/conflicts_gifts_pad_register/`)
- `domain/` : pure stdlib, no cloud/framework imports. `kernel.py` (vertical-neutral types,
  `StrEnum` taxonomies from the commons, and the `AuditEvent` that masks its own content at
  construction), `models.py` (the register artifacts), `pii.py` (the jurisdiction pattern
  selection + order), `screening_engine.py` (the deterministic engine), `assessment_service.py`
  (the orchestration), `tenancy.py` (the one definition of tenant isolation, read and write),
  `errors.py`.
