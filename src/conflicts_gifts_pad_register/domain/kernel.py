...
def redacted_citations(citations: tuple[Citation, ...]) -> tuple[Citation, ...]:
    ...
    (``declaration:<the client's own register id>``, see
    :meth:`~conflicts_gifts_pad_register.domain.screening_engine.ScreeningEngine.subject_citation`),
    so both are raw client text wearing a structural-looking name. No sink can tell an
    engine-built citation from an extractor-built one by inspection, so this masks
