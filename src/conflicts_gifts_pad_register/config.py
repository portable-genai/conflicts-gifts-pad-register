...
One env var (``CONFLICTSPAD_PROFILE``) selects the adapter family for every
...
from .ports.declarations import BrokerageFeedPort, DeclarationFeedPort
...
_PROFILE_ENV = "CONFLICTSPAD_PROFILE"
_SETTINGS_ENV = "CONFLICTSPAD_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``CONFLICTSPAD_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CONFLICTSPAD_PROFILE", "local")``: that fallback reads an
...
_PKG = "conflicts_gifts_pad_register"
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "llm": {
        "local": f"{_PKG}.adapters.local.llm:LocalLlmAdapter",
        "gcp": f"{_PKG}.adapters.gcp.llm:VertexLlmAdapter",
        "onprem": f"{_PKG}.adapters.onprem.llm:OnPremLlmAdapter",
    },
    "declaration_feed": {
        "local": f"{_PKG}.adapters.local.declarations:LocalDeclarationFeed",
        "gcp": f"{_PKG}.adapters.gcp.declarations:BigQueryDeclarationFeed",
        "onprem": f"{_PKG}.adapters.onprem.declarations:OnPremDeclarationFeed",
    },
    "brokerage_feed": {
        "local": f"{_PKG}.adapters.local.declarations:LocalBrokerageFeed",
        "gcp": f"{_PKG}.adapters.gcp.declarations:BigQueryBrokerageFeed",
        "onprem": f"{_PKG}.adapters.onprem.declarations:OnPremBrokerageFeed",
    },
    "reference_store": {
        "local": f"{_PKG}.adapters.local.reference_store:LocalReferenceStore",
        "gcp": f"{_PKG}.adapters.gcp.reference_store:BigQueryReferenceStore",
        "onprem": f"{_PKG}.adapters.onprem.reference_store:OnPremReferenceStore",
    },
    "register_store": {
        "local": f"{_PKG}.adapters.local.register_store:LocalRegisterStore",
        "gcp": f"{_PKG}.adapters.gcp.register_store:FirestoreRegisterStore",
        "onprem": f"{_PKG}.adapters.onprem.register_store:OnPremRegisterStore",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CONFLICTSPAD_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "CONFLICTSPAD_REVIEW_ROUTING"
...
class Settings:
    ...
    project_id: str = ""
    #: Adopter override for the screening threshold pack. Empty means the reference pack shipped
    #: at ``rulepacks/screening.yaml``; a bank points this at its own gifts-and-entertainment
    #: policy file. The pack loader is fail-closed, so a named-but-missing file refuses to start.
    screening_pack_path: str = ""
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            screening_pack_path=str(data.get("screening_pack_path") or ""),
...
class Container:
    ...
    @cached_property
    def declaration_feed(self) -> DeclarationFeedPort:
        adapter = self._bind("declaration_feed")
        assert isinstance(adapter, DeclarationFeedPort)
        return adapter

    @cached_property
    def brokerage_feed(self) -> BrokerageFeedPort:
        adapter = self._bind("brokerage_feed")
        assert isinstance(adapter, BrokerageFeedPort)
        return adapter

    @cached_property
    def reference_store(self) -> ReferenceStorePort:
        adapter = self._bind("reference_store")
        assert isinstance(adapter, ReferenceStorePort)
        return adapter
