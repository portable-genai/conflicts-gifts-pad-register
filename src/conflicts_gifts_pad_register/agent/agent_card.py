...
class AgentCard:
    ...
    provider: str = "conflicts-gifts-pad-register"
    skills: tuple[AgentSkill, ...] = field(default_factory=tuple)


SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="assess_declaration",
        name="Declaration screening",
        description=(
            "Screen one conflicts / gifts / personal-account-dealing declaration against the "
            "restricted list, the blackout calendar, the MNPI holdings and the per-role gift "
            "thresholds, record an already-redacted audit event, and ROUTE the assessment to "
            "human review when it is flagged (rule R8). Which rules fired and the severity are "
...
_DESCRIPTION = " ".join(
    (
        "Conflicts, Gifts and PAD Register",
...
def build_agent_card(settings: Settings | None = None) -> AgentCard:
    ...
    return AgentCard(
        name="conflicts-gifts-pad-register",
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://conflicts-gifts-pad-register.{settings.region}.internal.example/a2a"
