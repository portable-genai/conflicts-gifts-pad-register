...
``CONFLICTSPAD_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CONFLICTSPAD_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from ..domain.errors import TenantAccessDeniedError, UntaggedRegisterRecordError
from ..domain.tenancy import authorize_tenant_read
...
_INSECURE_DEMO_ENV = "CONFLICTSPAD_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CONFLICTSPAD_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CONFLICTSPAD_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CONFLICTSPAD_S2S_AUDIENCE"
_API_HOST_ENV = "CONFLICTSPAD_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CONFLICTSPAD_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CONFLICTSPAD_PROFILE`` must never be allowed to
    ...
    ``CONFLICTSPAD_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CONFLICTSPAD_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "conflicts-gifts-pad-register"
...
_APP_TITLE = "Conflicts, Gifts and PAD Register"
...
#
# Note what is NOT in this expression: CONFLICTSPAD_S2S_TOKEN. A service
...
def assess(
    ...
    A principal that carries NO tenant is refused with a 403 rather than filing an untagged
    record. A verified IAP token with no hosted-domain claim resolves to exactly that, and an
    untagged row is readable by every tenant.
    """
    container = _container()
    service = build_assessment_service(container)
    declaration = request.to_domain(tenant=principal.tenant)
    result = service.assess(declaration, actor=principal.actor)
    try:
        container.register_store.put(result)
    except UntaggedRegisterRecordError as exc:
...
@app.get("/v1/register/{assessment_id}", response_model=AssessResponse, tags=["artifacts"])
def get_assessment(
    assessment_id: str,
    principal: Annotated[Principal, Depends(get_principal)],
) -> AssessResponse:
    """Fetch one register entry, DENYING (403) any record this principal's tenant does not own.

    The store fetch is unfiltered by design; the comparison lives in ``domain/tenancy.py`` so
    every surface inherits the same answer. A denial is a 403, not a 404: a caller must not learn
    which ids exist in another tenant from the difference.

    A comparison of ``if record.tenant and record.tenant != principal.tenant``, and
    that leading truthiness was the defect this docstring claimed it was not: an UNTAGGED record
    skipped the comparison entirely and was readable by every tenant.
    ``authorize_tenant_read`` requires two identified tenants that are EQUAL, so an untagged
    record and a principal with no tenant are both denied.
    """
    container = _container()
    record = container.register_store.get(assessment_id)
    if record is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="assessment not found")
    try:
        authorize_tenant_read(record_tenant=record.tenant, principal_tenant=principal.tenant)
    except TenantAccessDeniedError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc
    return AssessResponse.from_domain(record)


@app.get(
    "/v1/reference/snapshot",
    response_model=ReferenceSnapshotResponse,
