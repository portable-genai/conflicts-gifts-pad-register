...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    CONFLICTSPAD_PROFILE=gcp \
    CONFLICTSPAD_SETTINGS=/app/config/settings.yaml \
    PORT=8080
...
CMD ["sh", "-c", "python -m conflicts_gifts_pad_register.managed_readiness && exec uvicorn conflicts_gifts_pad_register.api.app:app --host 0.0.0.0 --port ${PORT:-8080}"]
