...
Rewrites the python package name, the ``CONFLICTSPAD_`` env-var prefix, the baked-in cloud
...
* **The console script is named after the package.** ``pyproject.toml`` declares
  ``conflicts_gifts_pad_register = "conflicts_gifts_pad_register.cli.main:main"``, so there is
  no separate ``--cli`` knob: renaming the package renames the ``screen`` command too.
* **The env prefix has two spellings.** ``CONFLICTSPAD_`` prefixes every variable, and the bare
  token ``CONFLICTSPAD`` is carried as ``render_env_prefix`` in
...
screening pack, region, IdP, reference data, eval golden set) that this script does NOT do for
you.
"""
...
_OLD_PACKAGE = "conflicts_gifts_pad_register"
_OLD_ENV_PREFIX = "CONFLICTSPAD_"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    Order matters. The distribution id (``conflicts-gifts-pad-register``) is the longest and
    most specific string, so it goes first; the resource stem and the package name share no
    ...
    # The env prefix, only where it PREFIXES an upper token (CONFLICTSPAD_PROFILE), never
    # mid-word.
...
def main() -> int:
    ...
    print(f"  {_OLD_DIST!r:36} -> {(args.dist or args.resource)!r}   (distribution / git id)")
    print(f"  {_OLD_RESOURCE!r:36} -> {args.resource!r}   (Terraform name_prefix)")
    print(f"  {_OLD_PACKAGE!r:36} -> {args.package!r}   (package and console script)")
    print(
    ...
    print("Then finish the human steps in docs/ADOPTING.md (screening pack, region, IdP, eval).")
    return 0
