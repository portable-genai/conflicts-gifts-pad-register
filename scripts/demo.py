...
from conflicts_gifts_pad_register.domain import (
    kernel,
    models,
)
from conflicts_gifts_pad_register.domain.pii import (
    JURISDICTIONS,
)
...
SERVICE_NAME = "Conflicts, Gifts and PAD Register"
CATALOG_ID = "conflicts-gifts-pad-register"
REPOSITORY = "conflicts-gifts-pad-register"
...
#: A CLEAR declaration: an adviser's gift under the SG threshold. Nothing to escalate.
ROUTINE_CASE = models.Declaration(
    id="demo-gift-under",
    tenant=TENANT,
    employee="lim.adviser@bank.example",
    employee_role="adviser",
    market="SG",
    kind=models.DeclarationKind.GIFT,
...
#: A FLAGGED declaration: a personal-account deal on a restricted symbol. Escalates and routes.
ESCALATING_CASE = models.Declaration(
    id="demo-pad-restricted",
    tenant=TENANT,
    employee="chen.trader@bank.example",
    employee_role="trader",
    market="SG",
    kind=models.DeclarationKind.PERSONAL_ACCOUNT_DEAL,
    description="Buy 100 FICT via personal broker account.",
    as_of="2026-08-08",
    amount_minor=1500000,
    currency="SGD",
    instrument=models.Instrument(symbol="FICT", name="Fictus Holdings (FICTIONAL)"),
)

#: A FLAGGED gift that also carries a national id in the free text, for the redaction proof.
PII_CASE = models.Declaration(
    id="demo-gift-over",
    tenant=TENANT,
    employee="chen.trader@bank.example",
    employee_role="trader",
    ...
    kind=models.DeclarationKind.GIFT,
    description=(
        "Hamper from Vega Supplies (FICTIONAL); staff NRIC "
        + PLANTED_NRIC
        + " on the slip, reply-to ops@vega.example"
    ),
    as_of="2026-08-08",
...
class DemoRun:
    ...
    def _assess_panels(self, case: models.Declaration, *, expect_routing: bool) -> Produced:
        result = self.service.assess(case, actor=ACTOR)
        self.container.register_store.put(result)
        self.cases += 1
        review_ref = ""
        ...
        fired = ", ".join(f.rule_id for f in result.screening.findings) or "none"
        decision = Panel(
            ...
            rows=(
                ...
                Row("Rules fired", fired),
                Row("Requires human review", str(result.requires_human_review)),
                Row(
                    "Routed to review",
                    review_ref or "not routed (cleared)",
                    "ok" if consistent else "bad",
                ),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "Which rules fired and the verdict are computed by pure stdlib code and are "
                "replayable from the as-of snapshot. A model narrates this; it never produces it."
            ),
...
def _exit_audit(container: Any) -> Any:
    return container.audit.record(
        kernel.AuditEvent(
            ...
            redacted_summary="chen.trader (FICTIONAL): flagged",
        )
    )
...
def _exit_review(container: Any) -> Any:
    citation = kernel.Citation(
        source_id="declaration:demo",
        title="Declaration under screening",
        snippet="as_of 2026-08-08",
    )
    return container.review_router.route(
        models.ConflictAssessment(
            declaration_id="demo-pad-restricted",
            subject="chen.trader@bank.example / FICT",
            employee="chen.trader@bank.example",
            tenant=TENANT,
            market="SG",
            kind=models.DeclarationKind.PERSONAL_ACCOUNT_DEAL,
            verdict=models.AssessmentVerdict.ESCALATE,
            severity=kernel.Severity.HIGH,
            decision=kernel.Decision.ESCALATED,
            summary="personal_account_deal flagged as of 2026-08-08: restricted_list.",
            screening=models.ScreeningResult(
                declaration_id="demo-pad-restricted",
                outcome=models.ScreeningOutcome.FLAGGED,
                as_of="2026-08-08",
            ),
...
def _exit_reference(container: Any) -> Any:
    return container.reference_store.snapshot("2026-08-08")


def _exit_register(container: Any) -> Any:
    return container.register_store.get("demo-pad-restricted")


def _exit_declaration(container: Any) -> Any:
    return container.declaration_feed.declarations(TENANT)


def _exit_brokerage(container: Any) -> Any:
    return container.brokerage_feed.pad_trades(TENANT)
