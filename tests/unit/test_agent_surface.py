...
from conflicts_gifts_pad_register.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    assess_declaration,
...
def _call_tool(declaration: Declaration) -> dict[str, object]:
    return assess_declaration(
        ...
        symbol=declaration.instrument.symbol if declaration.instrument else "",
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "conflicts-gifts-pad-register"
...
def test_the_agent_path_routes_an_escalation_rather_than_only_flagging_it() -> None:
    result = _call_tool(sample_cases.FLAGGED_DECLARATION)
    assert result["requires_human_review"] is True
    assert result["review_ref"], "the agent surface flagged an escalation it never routed"


def test_the_agent_path_does_not_manufacture_a_review_for_a_cleared_case() -> None:
    result = _call_tool(sample_cases.CLEAN_DECLARATION)
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    data. The employee identifier travels on the assessment subject, so it must be masked.
    """
    result = _call_tool(sample_cases.FLAGGED_DECLARATION)
    rendered = repr(result)
    assert sample_cases.FLAGGED_DECLARATION.employee not in rendered
