...
from conflicts_gifts_pad_register.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "CONFLICTSPAD_AUDIT_ANCHOR"
...
def test_a_truncated_tail_is_detected_because_of_the_anchor(tmp_path: Path) -> None:
    audit = _record_three(_durable_settings(tmp_path, anchored=True)).audit
...
def test_without_the_anchor_the_same_truncation_is_invisible(tmp_path: Path) -> None:
    ...
    audit = _record_three(_durable_settings(tmp_path, anchored=False)).audit
    _truncate_tail(audit)
    report = audit.verify()
    assert report.ok, (
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    _truncate_tail(container.audit)
