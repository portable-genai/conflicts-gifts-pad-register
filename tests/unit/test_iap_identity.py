...
from conflicts_gifts_pad_register.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from conflicts_gifts_pad_register.ports.identity import (
    VERIFIED,
...
_API_MODULE = "conflicts_gifts_pad_register.api.app"
...
#: A valid, deterministically FLAGGING assessment body (a trader gift over the SG threshold),
#: hoisted to a constant so no call site carries it inline and blows the line-length limit.
_ASSESS_BODY: dict[str, object] = {
    "id": "t-exposure",
    "employee": "e@bank.example",
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # The rest of the ports are bound to their local adapters in every profile: this suite
        # is about the IDENTITY posture, so the other ports just need to resolve so the app
        # imports. The loader requires every declared port to appear, all three profiles each.
        "  llm:",
        *[
        ...
        *[
        ...
        "  brokerage_feed:",
        *[
            f"    {p}: {_PKG}.adapters.local.declarations:LocalBrokerageFeed"
            for p in ("local", "gcp", "onprem")
        ],
        "  reference_store:",
        *[
            f"    {p}: {_PKG}.adapters.local.reference_store:LocalReferenceStore"
            for p in ("local", "gcp", "onprem")
        ],
        "  register_store:",
        *[
            f"    {p}: {_PKG}.adapters.local.register_store:LocalRegisterStore"
            for p in ("local", "gcp", "onprem")
        ],
    ]
)
