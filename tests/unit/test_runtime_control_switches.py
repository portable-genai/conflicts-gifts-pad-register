...
control this service has: ``CONFLICTSPAD_REVIEW_ROUTING`` is read in three states; off binds a
disabled router and says so at startup; on under the managed profile refuses to boot without a
console; and the API, the agent tool and the CLI's screen command report ``review_routing``
rather than failing an already-computed assessment when the console is unreachable.
"""
...
from conflicts_gifts_pad_register.adapters.controls import (
    DisabledReviewRouter,
    RecordingReviewRouter,
    ReviewRouting,
)
from conflicts_gifts_pad_register.agent import tools
from conflicts_gifts_pad_register.api import app as api_module
from conflicts_gifts_pad_register.api.app import app
from conflicts_gifts_pad_register.cli import main as cli
...
from conflicts_gifts_pad_register.envread import ConfiguredEmptyError
...
_ESCALATING: dict[str, Any] = {
    ...
    "market": "SG",
    "kind": "gift",
    "description": "Hamper from Vega Supplies (FICTIONAL)",
    "as_of": "2026-08-08",
...
_LOCAL_ROUTER = "conflicts_gifts_pad_register.adapters.local.review_router.LocalReviewRouter.route"
...
def _managed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "conflicts_gifts_pad_register.config.resolve_profile",
...
def test_the_off_posture_is_logged_once_however_many_containers(
    ...
    settings = Settings(profile="local", controls=ControlSwitches(review_routing=False))
    with caplog.at_level(logging.WARNING, logger="conflicts_gifts_pad_register.config"):
        for _ in range(3):
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    failed = RecordingReviewRouter(_Refusing())
    with caplog.at_level(logging.WARNING, logger="conflicts_gifts_pad_register.adapters.controls"):
...
def test_the_agent_tool_reports_the_hand_off() -> None:
    payload = tools.assess_declaration(
        declaration_id="dec-flagged-agent-rcs",
        employee="chen.trader@bank.example",
        employee_role="trader",
        market="SG",
        kind="gift",
