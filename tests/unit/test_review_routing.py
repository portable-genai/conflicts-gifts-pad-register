"""Rule R8: a flagged assessment is ROUTED to Hrz7, not left in a per-repo boolean.
...
def test_the_payload_is_redacted_before_it_leaves_the_process() -> None:
    """Hrz7 is a shared sink; the employee identifier must never reach the wire in the clear."""
    router = LocalReviewRouter(_settings())
