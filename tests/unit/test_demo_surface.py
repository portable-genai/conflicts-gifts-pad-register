...
def test_the_demo_data_is_obviously_fictional() -> None:
    ...
    blob = " ".join(case.description + " " + case.counterparty for case in cases) + demo.ACTOR
    assert "FICTIONAL" in blob, "the demo declarations do not announce themselves as fake"
    ...
    # Every employee and party is at an .example domain, never a routable one.
    for case in cases:
        assert case.employee.endswith(".example")
