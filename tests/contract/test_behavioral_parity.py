...
from conflicts_gifts_pad_register.adapters._review_payload import (
    result_to_review,
)
...
def _assess(declaration: Any, **overrides: Any) -> tuple[Any, Any]:
    container = build_container(local_settings(**overrides))
    service = build_assessment_service(container)
    return container, service.assess(declaration, actor=sample_cases.ACTOR)
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    local, result = _assess(sample_cases.FLAGGED_DECLARATION)
    ...
    service = build_assessment_service(onprem)
    with pytest.raises(NotImplementedError):
