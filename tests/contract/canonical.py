...
CANONICAL_EVENT = AuditEvent(
    ...
    redacted_summary="chen.trader / Vega Supplies (FICTIONAL): flagged",
    citations=(Citation(source_id="declaration:dec-flagged", title="Declaration", snippet=""),),
)

#: The escalated assessment every review-router implementation is handed (rule R8's payload).
CANONICAL_RESULT = sample_cases.CANONICAL_ASSESSMENT
...
#: The as-of the reference feed is replayed at (inside every seeded window).
CANONICAL_AS_OF = sample_cases.AS_OF

#: A canonical narration prompt: the local model echoes the BASELINE, so it answers grounded.
CANONICAL_PROMPT = (
    "Draft a rationale. FINDINGS: gift_threshold.\n\nBASELINE: gift flagged as of 2026-08-08.\n\n"
    'Reply with JSON of the form {"rationale": "<text>"}.'
)
_RATIONALE_SCHEMA: dict[str, Any] = {"type": "object", "required": ["rationale"]}
...
def _llm_invoke(adapter: Any) -> Any:
    return adapter.generate(CANONICAL_PROMPT, schema=_RATIONALE_SCHEMA)


def _llm_answered(_adapter: Any, result: Any) -> bool:
    return isinstance(result, str) and "rationale" in result


def _declaration_invoke(adapter: Any) -> Any:
    return adapter.declarations(sample_cases.TENANT)


def _declaration_answered(_adapter: Any, result: Any) -> bool:
    return isinstance(result, tuple) and len(result) > 0


def _brokerage_invoke(adapter: Any) -> Any:
    return adapter.pad_trades(sample_cases.TENANT)


def _brokerage_answered(_adapter: Any, result: Any) -> bool:
    return isinstance(result, tuple) and len(result) > 0


def _reference_invoke(adapter: Any) -> Any:
    return adapter.snapshot(CANONICAL_AS_OF)


def _reference_answered(_adapter: Any, result: Any) -> bool:
    return result.as_of == CANONICAL_AS_OF and bool(result.restricted)


def _register_invoke(adapter: Any) -> Any:
    adapter.put(CANONICAL_RESULT)
    return adapter.get(CANONICAL_RESULT.declaration_id)


def _register_answered(_adapter: Any, result: Any) -> bool:
    return result is not None and result.declaration_id == CANONICAL_RESULT.declaration_id
...
CANONICAL_CALLS: dict[str, PortCase] = {
    ...
    "llm": PortCase(
        ...
        # The lazy `vertexai` import is the first thing the managed model does.
        managed_refusal=(ImportError,),
        detail="narrate a grounded rationale over a fixed verdict",
    ),
    "declaration_feed": PortCase(
        invoke=_declaration_invoke,
        answered=_declaration_answered,
        managed_refusal=(ImportError,),
        detail="serve the tenant's employee declarations",
    ),
    "brokerage_feed": PortCase(
        invoke=_brokerage_invoke,
        answered=_brokerage_answered,
        managed_refusal=(ImportError,),
        detail="serve the tenant's personal-account-dealing trades",
    ),
    "reference_store": PortCase(
        invoke=_reference_invoke,
        answered=_reference_answered,
        managed_refusal=(ImportError,),
        detail="serve the as-of restricted / blackout / MNPI snapshot",
    ),
    "register_store": PortCase(
        invoke=_register_invoke,
        answered=_register_answered,
        managed_refusal=(ImportError,),
        detail="store and fetch one conflict assessment",
    ),
    "tracer": PortCase(
