# Runbook: Conflicts, Gifts and PAD Register (`conflicts-gifts-pad-register`)

## Deploy (gcp)
1. `CONFLICTSPAD_PROFILE=gcp`, install `.[gcp]`, region `asia-southeast1`.
...
3. Ingress is fronted by IAP/Apigee; the app authenticates the S2S caller fail-closed
   (`CONFLICTSPAD_S2S_TOKEN` local, Google OIDC + allowlist secure).
4. **Set `CONFLICTSPAD_IAP_AUDIENCE`.** Without it the service starts, stays
...
`CONFLICTSPAD_IAP_AUDIENCE` is the IAP-protected resource the assertion must be
...
| Symptom | Cause | Fix |
|---|---|---|
| 503, detail names `CONFLICTSPAD_IAP_AUDIENCE` | no audience configured | set it to the protected resource above |
...
| `gcp` IAP assertion | `verified` | authenticated: the signature (against IAP's own key set), the audience (against `CONFLICTSPAD_IAP_AUDIENCE`), the expiry and the issuer are all checked before any claim is read |
...
1. **Nobody chose a profile.** `CONFLICTSPAD_PROFILE` is absent, so no end-user
...
2. **The `local` profile, chosen deliberately.** The seeded personas are a client-asserted
   ...
   `CONFLICTSPAD_S2S_TOKEN` is set. Setting that secret closes the S2S
...
- `add_loopback_exposure_guard` is registered at module scope in `api/app.py`, so it holds under
  `uvicorn conflicts_gifts_pad_register.api.app:app --host 0.0.0.0` (what the Dockerfile `CMD`
...
Set `CONFLICTSPAD_ALLOW_INSECURE_DEMO=1` to accept the exposure deliberately.
...
`CONFLICTSPAD_PROFILE` is read once, in `config.py`, and it has three states:
...
`CONFLICTSPAD_S2S_TOKEN`. With routing on and the URL unset, the managed profile REFUSES TO BOOT,
...
`CONFLICTSPAD_REVIEW_ROUTING` switches routing, read in three states: unset is on, `true`/`false`
...
`CONFLICTSPAD_AUDIT_ANCHOR` (read by `audit_anchor_path` in
`config/settings.yaml`) to a file on a DIFFERENT volume from
`CONFLICTSPAD_AUDIT_PATH`, ideally writable by a different principal. This is
not decoration:
