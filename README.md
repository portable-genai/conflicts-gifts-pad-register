...
conflicts_gifts_pad_register screen --tenant demo-bank
```
...
One env var, `CONFLICTSPAD_PROFILE`, selects the adapter family:
...
`CONFLICTSPAD_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`CONFLICTSPAD_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
| `pii-kit` | the jurisdiction PII pattern pack the register redacts declarations with |
...
**Name the profile.** `CONFLICTSPAD_PROFILE` has no default. Leaving it unset is
