...
- `src/conflicts_gifts_pad_register/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `CONFLICTSPAD_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `CONFLICTSPAD_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `CONFLICTSPAD_PROFILE` (mentioning it in a refusal message is wanted, reading
...
- **Redact before anything leaves, and BY CONSTRUCTION.** `AuditEvent.__post_init__` masks the
  summary and every citation field, so the WORM boundary holds for callers that forget and for
  surfaces added later; `adapters/_review_payload.py` does the same before the wire. Scan the
  CONTENT fields (summary, citations including `source_id`) and never `actor`: the actor is the
  verified principal and is an address by design, so a scan widened to a whole audit row can
  never go green and the next person relaxes the threshold instead of narrowing the scan.
- **Tenant isolation is EQUALITY between two identified tenants.** `domain/tenancy.py` owns it:
  a read needs the record's tag and the principal's tenant both present and equal (403, never
  404), and an untagged record cannot be written at all. Never write it as
  `record.tenant and record.tenant != principal.tenant`: that truthiness made every untagged row
  world-readable, and untagged rows were real.
